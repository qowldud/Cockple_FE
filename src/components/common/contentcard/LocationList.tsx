  ...
  return (
    ...
    >
      <div className="flex flex-col items-start">
        <p className={`body-md-500 ${textColor}`}>{mainAddress}</p> 
        <p className={`body-rg-500 ${textColor}`}>{subAddress}</p>
      </div>

      <div className="flex justify-end w-full">
        <div className="w-[6.6875rem] h-[1.75rem] flex items-center gap-[0.625rem]">
          <p className={`body-rg-400 ${textColor}`}>{showOnMapText}</p> 
          <ArrowRight className={`w-[1rem] h-[1rem] ${disabled ? 'text-[#C0C4CD]' : 'text-black'}`} /> 
        </div>
      </div>
    </div>
  );
