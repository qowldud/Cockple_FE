...
export const useContestRecord = () => {
  ...
  const [selectedForm, setSelectedForm] = useState<
    (typeof FORM_OPTIONS)[number] | null
  >(null);
  ...
  useEffect(() => {
    const initializeData = async () => {
      ...
      if (dataToSet) {
        ...
        if (dataToSet.type && typeMapReverse[dataToSet.type])
          setSelectedForm(typeMapReverse[dataToSet.type]);
        else if (Object.values(typeMapReverse).includes(dataToSet.type))
          setSelectedForm(dataToSet.type);

        const levelMapReverse: any = {
          NOVICE: "왕초심",
          BEGINNER: "초심",
          D: "D조",
          C: "C조",
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setPhotos(prev =>
        [...prev, ...images.map(img => img.fileUrl)].slice(0, 3),
      );
    } catch (err) {
      console.error(err);
      alert("이미지 업로드 실패");
  ...
  const handleSaveClick = async () => {
    ...
    try {
      ...
      const finalImgs: AddContestImgRequest[] = photos.map((url, index) => ({
      ...
      if (response.success) {
        ...
        const newId = isEditMode
          ? contestId
          : resData.contestId || resData.data?.contestId;
